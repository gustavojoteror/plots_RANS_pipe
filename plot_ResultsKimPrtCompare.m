...
turbMod       = {'SST_Sta','MK_Mod'};           % = {'VF','SST','MK','SA'};
turbModname   = {'SST','MK-$\rho$'};           % = {'V2F','SST','MK','SA'};%'$\overline{v^2}-f$'};
fluid         = {'30'};
fluidname     = {'$30~kW/m^2$'};  
tempMod       = {'Prt0_4','Prt0_9','Prt1','Prt1_4'};
tempMod_name  = {'$Pr_t=0.4$','$Pr_t=0.9$','$Pr_t=1.0$','$Pr_t=1.4$'};
...
for i=1:nturb
    subplot(1,nturb,(nturb+1)-i);
    %%%%%%%%%%%%%%%%%%%%%%%%
    % ploting experimental data
    m = mindex(1);
    h0= subplotit_paper(x(1:m,1),y(1:m,1),1,'o','k',markersize,fontsize,1);
    ...
    for j=1:ntemp    
        ...
        filename2 = sprintf('data/case2/Q_%s/%s/%s/',fluid{1},tempMod{j},turbMod{i});
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %title
    title(sprintf(titleSpec,turbModname{i})); %,'FontWeight','bold');
    ...
    if((putlegend==1) & (i==1))
        ...
        legend([h0 hh(1) hh(2) hh(3) hh(4) ],{'Exp. Data',tempMod_name{1}, tempMod_name{2}, tempMod_name{3}, tempMod_name{4}},'location', locationlegend,'fontsize',fontsize*0.85);
...
print(strcat('RANS_Kimdata_PrtCompare.eps'),'-depsc')
return
